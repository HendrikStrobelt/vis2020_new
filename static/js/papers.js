...
const card_image = (openreview, show) => {
    if (show) return ` <center><img class="lazy-load-img cards_img" data-src="static/thumbs/${openreview.content['pdf_local'].replace(
      '.pdf', '.png')}" width="80%"/></center>`
    else return ''
}
...
const card_detail = (openreview, show) => {
    if (show)
     ...
     <div class="pp-card-header">
        ...
        <p class="card-text"> ${openreview.content.abstract}</p>
</div>
    </div>
`
    else return ''
...
let link_creation = openreview => `paper_${openreview.id}.html`
...
const card_html = openreview => `
        <div class="pp-card pp-mode-` + render_mode + ` ">
            ...
            <div class="checkbox-paper ${openreview.content.read ? 'selected' : ''}" style="display: block;position: absolute; bottom:35px;left: 35px;">✓</div>    
                <a href="${link_creation(openreview)}"
                target="_blank"
                   class="text-muted">
